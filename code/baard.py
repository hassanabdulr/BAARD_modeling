...
def make_master_df():
    baard_dir = "/home/aabdulrasul/Projects/baard"
    ...
    # --- weekly med flags: make them clean Int64 (0/1 with NA support) ---
    for week in [2, 4, 6, 8, 10]:
        m1 = master_df.get(f"week{week}_med1")
        m2 = master_df.get(f"week{week}_med2")
        if m1 is None and m2 is None:
            master_df[f"on_bup_week{week}"] = pd.Series(0, index=master_df.index, dtype="Int64")
            master_df[f"on_arp_week{week}"] = pd.Series(0, index=master_df.index, dtype="Int64")
            continue
        m1 = m1.astype(str).str.upper()
        m2 = m2.astype(str).str.upper()
        master_df[f"on_bup_week{week}"] = ((m1 == "BUPROPION") | (m2 == "BUPROPION")).astype("Int64")
        master_df[f"on_arp_week{week}"] = ((m1 == "ARIPIPRAZOLE") | (m2 == "ARIPIPRAZOLE")).astype("Int64")
    ...
    master_df["total_on_bup"] = master_df[bup_cols].sum(axis=1).astype("Int64") if bup_cols else 0
    master_df["total_on_arp"] = master_df[arp_cols].sum(axis=1).astype("Int64") if arp_cols else 0


    master_df['medication_group'] = pd.Series(pd.NA, index=master_df.index, dtype='string')

    mask_bup = (master_df['total_on_bup'] >= 1) & (master_df['total_on_arp'] == 0)
    mask_arp = (master_df['total_on_arp'] >= 1) & (master_df['total_on_bup'] == 0)

    master_df.loc[mask_bup, 'medication_group'] = 'BUPROPION'
    master_df.loc[mask_arp, 'medication_group'] = 'ARIPIPRAZOLE'

    # downstream flags (nullable Int64 to tolerate NA in medication_group)
    master_df['taking_bup'] = (master_df['medication_group'] == 'BUPROPION').astype('Int64')
    master_df['taking_arp'] = (master_df['medication_group'] == 'ARIPIPRAZOLE').astype('Int64')
    # sum falls and injury info


    # --- falls & injuries: coerce to numeric before summing ---
    ...
    if falls_cols:
        ...
        master_df["total_number_falls"] = (
            master_df[falls_cols].sum(axis=1, skipna=True).fillna(0).astype("Int64")
        )
    else:
        master_df["total_number_falls"] = pd.Series(0, index=master_df.index, dtype="Int64")
    ...
    if inj_cols:
        ...
        master_df["total_number_injuries"] = (
            master_df[inj_cols].sum(axis=1, skipna=True).fillna(0).astype("Int64")
        )
    else:
        master_df["total_number_injuries"] = pd.Series(0, index=master_df.index, dtype="Int64")

    master_df["had_fall"] = (master_df["total_number_falls"] > 0).astype("Int64")
